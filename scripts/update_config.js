 ...
 *
 * Usage Example:
 * 		node scripts/update_config.js ../feel-backup/config.json ./config/mainnet/config.json
 *
 * 	Reference:
...
const { configurator } = require('feel-framework');
const {
	stringifyEncryptedPassphrase,
	getPrivateAndPublicKeyFromPassphrase,
	encryptPassphraseWithPassword,
} = require('@feelhq/feel-cryptography');
...
program
	...
	.option('-n, --network [network]', 'Specify the network or use FEEL_NETWORK')
...
const network = program.network || process.env.FEEL_NETWORK;
...
const history = new JSONHistory('feel config file', console);
...
history.migrate(
	...
	(err, json) => {
		...
		process.env.FEEL_CONFIG_FILE = tempFilePath;

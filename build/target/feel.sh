#!/bin/bash
#
# feelfoundation/feel-scripts/feel.sh
...
if [ ! -f "$PWD/.build" ]; then
	echo "Error: Feel installation was not found. Exiting."
	exit 1
fi

if [ "$UID" == 0 ]; then
	echo "Error: Feel should not be run be as root. Exiting."
	exit 1
fi
...
PM2_CONFIG="$PWD/etc/pm2-feel.json"

function get_feel_app_name() {
	local PM2_CONFIG=$1
	PM2_APP="$( jq .apps[0].name -r "$PM2_CONFIG" )"
	echo "$PM2_APP"
}
PM2_APP=$( get_feel_app_name "$PM2_CONFIG" )

function get_feel_custom_config() {
	local PM2_CONFIG=$1
	local REGEXP="-c ([^ ]+)"
	PM2_APP_ARGS="$( jq .apps[0].args -r "$PM2_CONFIG" )"
	if [[ "$PM2_APP_ARGS" =~ $REGEXP ]]; then
		FEEL_CUSTOM_CONFIG="${BASH_REMATCH[1]}"
	else
		FEEL_CUSTOM_CONFIG=/dev/null
	fi
	echo "$FEEL_CUSTOM_CONFIG"
}
FEEL_CUSTOM_CONFIG=$( get_feel_custom_config "$FEEL_PATH/etc/pm2-feel.json" )

function cleanup() {
	rm -f "$RUNNING_CONFIG"
}
RUNNING_CONFIG=$( mktemp --tmpdir="$FEEL_PATH" )
trap cleanup INT QUIT TERM EXIT

function dump_running_config() {
	node scripts/generate_config.js --config "$FEEL_CUSTOM_CONFIG" >"$RUNNING_CONFIG"
}
dump_running_config
...
FEEL_LOGS=$( get_config '.components.logger.logFileName' )
LOGS_DIR="$PWD/logs"
...
network() {
	NETWORK=${FEEL_NETWORK%net}
	echo "Feel configured for $FEEL_NETWORK" |tee -a "$SH_LOG_FILE"
}
...
frobnicate() {
	# if not custom URL has been passed, use downloads.feel.surf
	if [ -z "$BLOCKCHAIN_URL" ]; then
		...
		if [ "$NETWORK" != "main" ] && [ "$NETWORK" != "test" ] && [ "$NETWORK" != "beta" ]; then
			...
			BLOCKCHAIN_URL="https://downloads.feel.surf/feel/$NETWORK"
		fi
	fi
}
...
autostart_cron() {
	...
	crontab=$($cmd -l 2> /dev/null | sed '/feel\.sh start/d' 2> /dev/null)

	crontab=$(cat <<-EOF
		$crontab
		@reboot $(command -v "bash") $PWD/feel.sh start > $PWD/cron.log 2>&1
EOF
	)
...
start_feel() {
	...
	if pm2 start "$PM2_CONFIG"  >> "$SH_LOG_FILE"; then
		echo "[+] Feel started successfully."
		sleep 3
		check_status
	else
		echo "[-] Failed to start Feel."
	fi
}

set_logs_paths() {
	mkdir -p "$(pwd)/logs/$FEEL_NETWORK"
	TEMP=$( mktemp )
	jq '.apps[0].out_file = "logs/'"$FEEL_NETWORK"'/feel.app.log"|.apps[0].error_file = "logs/'"$FEEL_NETWORK"'/feel.app.err"' "$PM2_CONFIG" >"$TEMP"
...
stop_feel() {
	...
	if [ "$STATUS" -eq 0 ] ; then
		echo "[+] Feel stopped successfully."
	else
		echo "[+] Feel is not running"
	fi
	stop_redis
}

reload_feel() {
	echo "Stopping Feel to reload PM2 config"
	stop_feel
	start_feel
}
...
check_status() {
	get_status
	if [ "$STATUS" -eq 0  ]; then
		echo "[+] Feel is running as PID: $PID"
		blockheight
	else
		echo "[-] Feel is not running"
		exit 1
...
help() {
	echo -e "\\nCommand Options for Feel.sh"
	echo -e "\\nAll options may be passed [-p <PM2-config.json>]"
	echo -e "\\nstart_node                         Starts a Nodejs process for Feel"
	echo -e "start                                 Starts the Nodejs process and PostgreSQL Database for Feel"
	echo -e "stop_node                             Stops a Nodejs process for Feel"
	echo -e "stop                                  Stop the Nodejs process and PostgreSQL Database for Feel"
	echo -e "reload                                Restarts the Nodejs process for Feel"
	...
	echo -e "coldstart                             Creates the PostgreSQL database and configures config.json for Feel"
	echo -e "feel                                  Launches Feel-commander"
	echo -e "logs                                  Displays and tails logs for Feel"
	echo -e "status                                Displays the status of the PID associated with Feel"
		...
		case "$OPT" in
			p)
				if [ -f "$OPTARG" ]; then
					PM2_CONFIG="$OPTARG"
					PM2_APP=$( get_feel_app_name "$PM2_CONFIG" )
					FEEL_CUSTOM_CONFIG=$( get_feel_custom_config "$PM2_CONFIG" )
					dump_running_config
...
*)
	...
	echo ""
	echo "Available commands are: start stop start_node stop_node start_db stop_db reload rebuild coldstart logs feely status help"
	help
	;;
esac

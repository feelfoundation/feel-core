...
const { TO_FEEL, TO_FELLOWS, from, sortBy, flattern } = require('../utils');
const FeelUtil = require('./feel_util');

/* eslint camelcase: ["error", {allow: ["codecept_helper"]}] */
const Helper = codecept_helper;
const feelUtil = new FeelUtil();
...
class ValidateHelper extends Helper {
	...
	async haveAccount(params) {
		const { result, error } = await from(feelUtil.call().getAccounts(params));
		const {
			data: [account],
		} = result;
	...
	async haveAccountWithBalance(address, balance) {
		...
		if (!account || !(TO_FEEL(account.balance) >= balance)) {
			if (account) {
				balance = Math.ceil(balance - TO_FEEL(account.balance));
			}

			await feelUtil.transfer({
				recipientId: address,
				amount: TO_FELLOWS(balance),
			});
		}
	...
	async haveAccountWithSecondSignature(address, passphrase, secondPassphrase) {
		...
		if (account && account.secondPublicKey) {
			...
			await feelUtil.registerSecondPassphrase(passphrase, secondPassphrase);
		}
		return account;
	}
	...
	async haveAccountRegisteredAsDelegate(params) {
		...
		if (account && account.delegate) {
			...
			await feelUtil.registerAsDelegate(params);
		}
		return account;
	}
	...
	async haveMultiSignatureAccount(requester, keepers, params) {
		...
		await feelUtil.registerMultisignature(keepers, params);
		return account;
	}

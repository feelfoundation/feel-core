...
Then('I transfer {int}F39 to account {string} from genesis account', async (amount, user) => {
...
Then('I transfer {string}F39 from account {string} to {string}', async (amount, fromAcc, toAcc) => {
...
Then('I transfer {string}F39 from second signature account {string} to {string}', async (amount, fromAcc, toAcc) => {
	const user = `${fromAcc}to${toAcc}`;
	const trx = await I.transfer({
		recipientId: getUserAddress(userAccounts, toAcc),
		amount: TO_FELLOWS(amount),
...
Then('I expect transfer {string}F39 from {string} to {string} should succeeded', async (amount, fromUser, toUser) => {
	...
	expect(result.data[0].amount).to.deep.equal(TO_FELLOWS(amount));
});

Then('I expect transfer {string}F39 from {string} to {string} should fail', async (amount, fromUser, toUser) => {
	const api = await I.call();
	const { id } = transactions.find(t => t.action === `${fromUser}to${toUser}`);
